...
### 战略知识建模
战略是复杂推理，符号表示与向量表示都难以直接描述策略的应用关系。StrategyKG 基于认知科学中的"抽象阶梯"定义出战略知识的四个层次，使战略知识建模呈现立体化，以进一步探索复杂推理的应用。

| 层次    | 层次名称    | 抽象级别 | 知识类型  | 时间维度 |
| ----- | ----- | ---- | ----- | ---- |
| 第0层   | 概念层   | 高阶抽象 | 理论性知识 | 无时间性 |
| 第1层   | 分类层 | 中阶抽象 | 领域知识  | 长期趋势 |
| 第2层   | 实体层   | 低阶抽象 | 事实性知识 | 历史时态 |
| 第3层   | 动态层   | 具体实例 | 过程性知识 | 实时流  |

## StrategyKG 成熟度模型
构建知识图谱是一项复杂且长期的工作，需要大量准确的文档和有效的数据集。为了满足不同用户的需求并推动 StrategyKG 的成长，我们采用成熟度模型来定义项目的发展路径。StrategyKG 成熟度模型包含四个级别，每个级别都针对不同的用户需求，并在不同的许可协议下提供特定的内容。
